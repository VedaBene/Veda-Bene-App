...
- **Proteção contra Tradução Automática e Autofill (DOM)**: Em `Input.tsx`, `LoginPage` e `ServiceOrderListTable.tsx`, textos dinâmicos e rótulos são envolvidos em tags `<span>` e exceções de DOM causadas por extensões/preenchimento automático/Google Translate são ignoradas em `sentry.client.config.ts` (`JAVASCRIPT-NEXTJS-G` e `JAVASCRIPT-NEXTJS-5`).
