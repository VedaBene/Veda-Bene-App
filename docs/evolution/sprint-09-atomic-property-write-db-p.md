# Dossiê DB-P — Sprint 09: Escrita Atômica de Propriedade e Relações
...
- **Motivo:** Eliminar o risco de criação de agências ou proprietários órfãos e propriedades parciais quando a persistência falha em etapas intermediárias. Unificar a resolução/criação de agência ou proprietário e a inserção/atualização do imóvel em uma única unidade transacional atômica no PostgreSQL.
- **Objetos Criados:**
  - `public.save_property_atomic(uuid, text, text, text, text, text, text, text, uuid, text, text, text, uuid, text, text, text, numeric, numeric, numeric, integer, integer, integer, integer, integer, integer, integer, integer, integer, integer, numeric, numeric, numeric, text)`
- **Objetos Modificados/Removidos:** Nenhum.
- **Tabelas, Colunas e Policies:** Preservadas integralmente sem alterações.

---

## 2. Medições de Linha de Base (Baseline de Dados)

Inspeção somente-leitura realizada na base do projeto em 2026-08-20:
- **Total de agências (`agencies`):** 1
- **Total de proprietários (`owners`):** 20
- **Total de imóveis (`properties`):** 179
- **Emails duplicados em agências:** 0
- **Nomes duplicados em agências:** 0
- **Emails duplicados em proprietários:** 0
- **Nomes duplicados em proprietários:** 0
- **Agências órfãs:** 0
- **Proprietários órfãos preexistentes:** 1 (proprietário `WINTIME`, id `0fe51b5d-99ca-4f8b-8db8-9c33197ac554`, `email: null`)
- **Imóveis rental sem agência vinculada:** 0
- **Imóveis particular sem proprietário vinculado:** 0

> [!NOTE]
> Conforme a regra de integridade de dados (`docs/production-data-safety.md`), o registro órfão preexistente é preservado intacto e não é excluído ou alterado automaticamente.

---

## 3. Comandos SQL da Migração

```sql
-- Migration: Atomic property and relations write procedure
-- Sprint 09 — DB-P
...
-- 1. Preconditions (Drift Detection)
DO $preconditions$
BEGIN
...
## 4. Locks, Timeouts e Concorrência

- `lock_timeout = '5s'` e `statement_timeout = '60s'`.
- Na criação: insert direto do registro de agência/proprietário e da propriedade dentro da transação.
- Na edição: lock de linha exclusivo `SELECT ... FOR UPDATE` no imóvel e na relação preexistente (`agencies` ou `owners`) antes das atualizações.
- Não realiza chamadas de rede, Auth, Storage ou Sentry dentro da transação.

---

## 5. Invariantes de Dados

- Contagem de linhas e integridade referencial de `properties`, `agencies`, `owners` e `profiles` permanecem idênticos antes e após a migração DDL.
- O guard da Sprint 04 e os grants restritos da Sprint 05 permanecem 100% preservados.

---

## 6. Rollback Não Destrutivo

- Como a migração adiciona estritamente uma função (`public.save_property_atomic`), o rollback de aplicação é a simples reimplantação da versão de código anterior.
- A função no banco permanece inócua sem efeitos colaterais caso a aplicação anterior esteja em execução.

---

## 7. Registro de Aplicação Remota em Produção

- **Data de Execução:** 2026-08-20
- **Projeto Alvo:** Supabase `iwrbeiqqsvzhiuhkqnqg`
- **Autorização:** Confirmada expressamente pelo usuário após backup e PITR ativos.
- **Migration Aplicada:** `20260820230000_atomic_property_write.sql`
- **Pós-Validação Remota:**
  - `function_created`: `true`
  - `agency_count`: 1 (inalterado)
  - `owner_count`: 20 (inalterado)
  - `property_count`: 179 (inalterado)
  - Grants de execução restritos a `authenticated`, `postgres` e `service_role`.
  - Supabase Security Advisor executado sem qualquer novo apontamento ou alerta.
