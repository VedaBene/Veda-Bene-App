...
| `service_orders` | `pricing_mode`, `extra_services_price` | `admin`, `secretaria`, `limpeza` apenas no recálculo autorizado da O.S. visível | Recalcular `total_price` server-side | `loadAuthorizedOrderPricingContext` | `OrderPricingContext`; nenhum valor-base é retornado ao navegador | `recalculateOrderPricing` e actions de O.S. |
