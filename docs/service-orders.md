...
- Para os perfis operacionais `limpeza` (Pulizia) e `consegna`, a listagem mostra
  ...
  igual a hoje em `Europe/Rome`. O limite é superior: ordens atrasadas e o
  histórico continuam visíveis; ordens sem data e ordens de amanhã em diante
  são ocultadas. A regra também protege acesso direto por ID e API por meio do
  RLS, conforme o [ADR 016](decisions/016-visibilidade-equipe-operacional-somente-ate-hoje.md).
- Os filtros por funcionário são ocultados para Pulizia e Consegna. Seus filtros
  ...
  ampliar a janela autorizada além de hoje.
...
- Para Pulizia e Consegna, o cabeçalho dos PDFs ativos sem período manual informa
  explicitamente o limite até hoje, em vez de indicar “Tutte le date”.
...
- **Opcionalidade de Check-in, Check-out e Data Pulizia**: Os três campos de visita (`cleaning_date`, `checkout_at` e `checkin_at`) são inteiramente opcionais tanto para o perfil **Secretária** quanto para **Administrador**. O formulário utiliza a diretiva `noValidate` para evitar que navegadores bloqueiem a submissão com erros nativos de `badInput` caso o usuário selecione uma data sem hora. Há botões de limpeza rápida ao lado de cada campo e sanitização defensiva no backend (`optIsoDateStr`), persistindo `NULL` no banco de dados quando vazios ou incompletos.
- O cálculo do Modo Diário no servidor (todayStr) utiliza o fuso `Europe/Rome`.
- O teto de visibilidade da equipe operacional é o próprio dia de Roma,
  calculado no DAL e no PostgreSQL. A página é atualizada ao voltar ao foco após uma
